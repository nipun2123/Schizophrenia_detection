...
def tf_plot_save(tfs,ty, label):
    ...
    for tf in tfs:
            ...
            if(ty=='training'):
                if(label=='healthy'):
                    plt.savefig('dataset_occipital/training/healthy/h'+str(no_round)+'.png', pad_inches=-0.1, bbox_inches = 'tight')
                    plt.close()
                else:
                    plt.savefig('dataset_occipital/training/sc/sc'+str(no_round)+'.png',pad_inches=-0.1, bbox_inches = 'tight')
                    plt.close()
            elif(ty=='testing'):
                if(label=='healthy'):
                    plt.savefig('dataset_occipital/testing/healthy/h'+str(no_round)+'.png',pad_inches=-0.1, bbox_inches = 'tight')
                    plt.close()
                else:
                    plt.savefig('dataset_occipital/testing/sc/sc'+str(no_round)+'.png',pad_inches=-0.1, bbox_inches = 'tight')
                    plt.close()

            elif(ty=='val'):
                if(label=='healthy'):
                    plt.savefig('dataset_occipital/validation/healthy/h'+str(no_round)+'.png',pad_inches=-0.1, bbox_inches = 'tight')
                    plt.close()
                else:
                    plt.savefig('dataset_occipital/validation/sc/sc'+str(no_round)+'.png',pad_inches=-0.1, bbox_inches = 'tight')
                    plt.close()
